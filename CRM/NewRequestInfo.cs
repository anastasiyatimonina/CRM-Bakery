...
namespace CRM
{
    public partial class NewRequestInfo : Form
    {
        ...
        private void NewRequestInfo_Load(object sender, EventArgs e)
        {
            if (r == 1)
            {
            ...
            while (reader1.Read())
            {
                ...
                textBox2.Text = reader1[4].ToString();
            }
            reader1.Close();
        ...
        {
            string queryString2 = $"UPDATE Request SET State = 'В работе' WHERE RequestId = {value}";
            SqlCommand command2 = new SqlCommand(queryString2, database.getConnection());
            database.openConnection();
